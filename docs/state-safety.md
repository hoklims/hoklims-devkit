...
Directory identity checks use a stable creation timestamp when the filesystem exposes one. Without it, the launcher conservatively uses change metadata to detect reused identities. On those filesystems, unrelated activity in a shared ancestor can still cause a refusal; retry from a quieter local state path rather than bypassing the check.
